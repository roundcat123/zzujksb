...
headers0 = {
	...
	'Referer': 'https://jksb.v.zzu.edu.cn/vls6sss/zzujksb.dll/', #后续修改此处即可
...
sid = login_response.text[pos2+5:pos2+23]
...
form = requests.post('https://jksb.v.zzu.edu.cn:443/vls6sss/zzujksb.dll/jksb', headers=requestform_headers, data=requestform_data)
...
submit_data = {
	...
	"myvs_14": userinfo['当日返郑'],
	"myvs_14b": userinfo['返回前居住地和抵郑时间'],
	"memo22": "无法获取",
	"did": "2",
	"door": '',
	"day6": "b",
...
#判断是否成功
if submit_response.text.find('zzujksb.dll/endok') != -1:
